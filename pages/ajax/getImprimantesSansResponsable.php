...
try {
    ...
    $bdd = User::getBDD();
    $query = "SELECT * FROM copieurs WHERE BDD = '$bdd' AND `N° de Série` NOT IN (SELECT `numéro_série` FROM users_copieurs)";
    ...
    $sel = $pdo->prepare("SELECT count(*) as allcount FROM copieurs
                        WHERE BDD = '$bdd' AND `N° de Série` NOT IN
                        (SELECT `numéro_série` FROM users_copieurs) " . $searchQuery);
    $sel->execute($option);
    $records = $sel->fetch();
